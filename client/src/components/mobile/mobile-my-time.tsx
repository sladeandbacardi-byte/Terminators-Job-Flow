    ...
    >
      ...
      {mode !== "none" && (
        <form onSubmit={submit} className="space-y-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
          ...
          {form.startTime && form.finishTime && <div className={`rounded-lg border p-3 text-sm ${total && total > 0 ? "border-gray-200 bg-gray-50 text-gray-700" : "border-amber-200 bg-amber-50 text-amber-800"}`}>{total && total > 0 ? <>{mode === "overtime" ? "Calculated overtime" : "Total Time Off"}: <strong>{formatOvertimeMinutes(total)}</strong></> : "No normal working time exists in this period."}</div>}
          <Button type="submit" disabled={saving || !total || total <= 0} className={mode === "overtime" ? "bg-red-600 hover:bg-red-700" : "bg-gray-900 hover:bg-gray-800"}>{saving ? "Saving…" : mode === "timeoff" ? "Submit Time Off" : "Submit Overtime"}</Button>
        </form>
      )}
