    ...
    <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-md flex-col justify-center space-y-6">
      ...
      <main className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        {error && <div className="mb-3 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</div>}
         {loading ? <div className="flex items-center justify-center gap-2 py-8 text-sm text-gray-500"><Loader2 className="h-4 w-4 animate-spin" />Loading staff…</div> : selected ? <form className="space-y-4" onSubmit={event => { event.preventDefault(); void signIn(selected); }}><button type="button" className="text-sm font-semibold text-gray-600 hover:text-red-600" onClick={() => { setSelected(null); setError(""); }}><ArrowLeft className="mr-1 inline h-4 w-4" />Switch user</button><div className="rounded-xl bg-red-50 p-4"><p className="font-semibold text-red-900">{selected.name}</p><p className="text-sm text-red-700">{selected.role} · {selected.department}</p></div><p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">PIN login has been disabled. Confirm your profile to continue.</p><button type="submit" disabled={Boolean(signingIn)} className="flex h-12 w-full items-center justify-center rounded-md bg-red-600 font-semibold text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50">{signingIn ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Signing in…</> : "Continue to staff dashboard"}</button></form> : <div className="space-y-2">{staff.map(profile => <button key={profile.id} type="button" onClick={() => { setSelected(profile); setError(""); }} className="flex w-full items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 text-left shadow-sm transition hover:border-red-400 hover:bg-red-50 hover:shadow-md"><div className="rounded-full bg-red-100 p-2.5 text-red-700"><UserRound className="h-5 w-5" /></div><div><p className="font-semibold text-gray-900">{profile.name}</p><p className="text-sm text-gray-600">{profile.role}</p><p className="text-xs uppercase tracking-wide text-gray-400">{profile.department}</p></div></button>)}</div>}
      </main>
