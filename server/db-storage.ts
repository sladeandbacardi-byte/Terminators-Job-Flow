...
export class DbStorage implements IStorage {
  ...
  private async seedDatabase(): Promise<void> {
    ...
    const workerRows = [
      ...
      { name: "sales2",              email: "sales2@terminators.co.za",    phone: "+27 82 770 0028", departmentId: "div-5", role: "Sales Rep" },
  ...
  async createInventoryItem(data: InsertInventoryItem): Promise<InventoryItem> {
    const [row] = await db.insert(inventoryItems).values({ id: randomUUID(), ...data, createdAt: new Date() }).returning();
    return row;
  }

  async updateInventoryItem(id: string, data: Partial<InsertInventoryItem>): Promise<InventoryItem> {
    const [row] = await db.update(inventoryItems).set(data).where(eq(inventoryItems.id, id)).returning();
